...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating InsuranceCompany1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-InsuranceCompany1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating InsuranceCompany2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-InsuranceCompany2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/InsuranceCompany1/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createInsuranceCompany1

    infoln "Creating InsuranceCompany2 Identities"

    createInsuranceCompany2
  ...
  infoln "Generating CCP files for InsuranceCompany1 and InsuranceCompany2"
...
# call the script to create the channel, join the peers of InsuranceCompany1 and InsuranceCompany2,
...
function networkDown() {
  # stop org3 containers also in addition to InsuranceCompany1 and InsuranceCompany2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/InsuranceCompany1/msp organizations/fabric-ca/InsuranceCompany1/tls-cert.pem organizations/fabric-ca/InsuranceCompany1/ca-cert.pem organizations/fabric-ca/InsuranceCompany1/IssuerPublicKey organizations/fabric-ca/InsuranceCompany1/IssuerRevocationPublicKey organizations/fabric-ca/InsuranceCompany1/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/InsuranceCompany2/msp organizations/fabric-ca/InsuranceCompany2/tls-cert.pem organizations/fabric-ca/InsuranceCompany2/ca-cert.pem organizations/fabric-ca/InsuranceCompany2/IssuerPublicKey organizations/fabric-ca/InsuranceCompany2/IssuerRevocationPublicKey organizations/fabric-ca/InsuranceCompany2/fabric-ca-server.db'
